...
preprocessLevel4 <- function(dt, seNames=NULL){
  ...
  #Merge back in the replicate metadata
  metadataNames <- grep("_SE|Barcode|^BW$|ArrayRow|ArrayColumn|^Well$|WellIndex|^Spot$|^PrintSpot$|^Well_Ligand$|ImageID|QA_|ECMSet|^Row$|^Column$|^Block$|PlateRow|^QAScore$|^ID$|PrintOrder",colnames(dt), value=TRUE,invert=TRUE) %>%
