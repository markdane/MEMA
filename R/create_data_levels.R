...
summarizeToSpot <- function(dt, seNames=NULL){
  ...
  metadataNames <- grep("(Row|Column|PrintOrder|Block|^ID$|Array|CellLine|Ligand|Drug|Endpoint|ECMp|MEP|Well_Ligand|ECM|ImageID|Barcode|^Well$|^WellIndex$|^PrintSpot$|^Spot$|Pin|Lx|[[:digit:]]{3}nm$|StainingSet|Density|^QA_|^CPQi|^Robust_|Cell_Singles|Cell_Doublets|^Cells_in|_Debris$|Fix|IncubationTime|$Media^)", x=colnames(dt), value=TRUE)
